...
function [obj,cost_value] = BPM_update(obj, psz, efield, efield_vol, amp_acqs, prop_phs, NA_crop, lambda, z_plane, step_size, pdar)
    
    % compute residual
    res                     = abs(efield) - abs(amp_acqs);
    % compute cost
    cost_value              = norm(res(:))^2;
    
    %% compute gradient 
    back_prop               = efield - abs(amp_acqs).* exp(1i*angle(efield));
