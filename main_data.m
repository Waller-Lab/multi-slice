...
plot_range      = [-0.04,0.08];  % contrast to be used to show the reconstruction at each iteration
...
for iter = 1:maxiter
    ...
    for illum_angle = 1:length(fx_in)
        ...
        [reconObj,funcVal]      = BPM_update(reconObj, psz, efield, efield_vol, amp_acqs(:,:,seq(illum_angle)), prop_phs, NA_crop, lambda, z_plane, step_size, pdar, use_field);
